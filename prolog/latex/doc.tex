...
The \predref{handle_input}{2} predicate preprocesses the input and tries to parse it as either a
...
The \predref{prove_question_list}{2} predicate is equivalent to \predref{prove_question}{2}, except it
...
\end{arguments}
    ...
    \predicate[private]{prove_from_known_facts}{4}{+Clause:atom, +FactList:list, -ProofList:list, -Proof:atom}
...
\end{arguments}
    ...
    \predicate[private]{prove_from_known_facts}{2}{+Clause:atom, +FactList:list}
The \predref{prove_from_known_facts}{2} predicate is equivalent to \predref{prove_from_known_facts}{4},
...
The \predref{output_proof_list}{2} predicate transforms a question and its proof list into a
string output.
...
\end{arguments}
    ...
    \dcg[private]{adjective}{2}{?Number:atom, ?X:atom}
The \dcgref{adjective}{2} DCG rule defines adjectives.
It relates the adjective's grammatical number and its literal.

\begin{arguments}
\arg{\Squest} & \arg{Number} The grammatical number. \\
\arg{\Squest} & \arg{X} The adjective \Sssu{} literal.
  \\
\end{arguments}

    \dcg[private]{noun}{2}{?Number:atom, ?X:atom}
The \dcgref{noun}{2} DCG rule defines common nouns.
It relates the noun's grammatical number and its literal.

\begin{arguments}
\arg{\Squest} & \arg{Number} The grammatical number. \\
\arg{\Squest} & \arg{X} The noun \Sssu{} literal.
  \\
\end{arguments}

    \dcg[private]{intransitive_verb}{2}{?Number:atom, ?X:atom}
...
\begin{arguments}
\arg{\Squest} & \arg{Number} The grammatical number. \\
\arg{\Squest} & \arg{X} The intransitive verb \Sssu{} literal.
  \\
\end{arguments}
...
\end{arguments}
    ...
    \predicate{predicate_to_grammar}{4}{+Predicate:atom, +Arity:integer, +WordCategory:atom, -XLiteral:callable}
...
\end{arguments}
    ...
    \predicate{try}{1}{+X}
The \predref{try}{1} predicate tries to prove a goal by negation-as-failure.

\begin{arguments}
\arg{X} & The goal to prove. \\
\end{arguments}

    \predicate[private]{write_debugs}{1}{+List}
The \predref{write_debugs}{1} predicate writes a list of debug messages to the console.

\begin{arguments}
\arg{List} & : A list of debug messages. \\
\end{arguments}
